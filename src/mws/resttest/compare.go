...
func compare(struct1 interface{}, struct2 interface{}) (areEqual bool, differences []*CompareResult, err error) {
	return comparePartial(struct1, struct2, nil)
}

func comparePartial(struct1 interface{}, struct2 interface{}, compFields []string) (areEqual bool, differences []*CompareResult, err error) {
	...
	//log.Printf("Val 1 = %v Val 2 = %v", v1, v2)
	...
	for i, numFields := 0, v1.NumField(); i < numFields; i++ {
		...
		if len(compFields) > 0 {
			if !contains(compFields, fieldType.Name) {
				log.Printf("Not Evaluating value of %s", fieldType.Name)
				continue
			} else {
				log.Printf("Evaluating value of %s", fieldType.Name)
			}
		}
		//log.Printf("field1 = %v, field2 = %v", field1, field2)
		switch val1, val2 := field1.Interface(), field2.Interface(); val1.(type) {

		case int, int64, bool, string, float64, time.Time:

			//log.Printf("val1 = %v, val1 = %v", val1, val2)
			if val1 != val2 {
		...
		default:
			...
			if field1.Kind() == reflect.Struct {
				log.Printf("Recursing type = %s", val1)
				pass, diff, err := comparePartial(val1, val2, compFields)
				if !pass || err != nil {
					for _, d := range diff {
						differences = append(differences, d)
						return false, differences, fmt.Errorf("%s  couldn't' match  %s", field1, field2)
					}
				}
				continue
			}
			if reflect.DeepEqual(val1, val2) {
				...
				result := &CompareResult{FieldName: fieldType.Name, Value1: val1, Value2: val2}
				differences = append(differences, result)
				return false, differences, fmt.Errorf("%s  to match of %s", field1, field2)
			}

			//v1, _ := toString(val1)
			//v2, _ := toString(val2)
			//log.Printf("val1 = %v, val1 = %v", v1, v2)
			//if v1 != v2 {
			log.Printf("Compare type = %s", val1)
			return false, differences, fmt.Errorf("Unsupported type: %v", val1)
			//}
		}
